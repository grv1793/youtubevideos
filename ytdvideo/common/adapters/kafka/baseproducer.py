...
from common.adapters.kafka.types import TMessage
from common.adapters.kafka.retry import retry, producer_on_fail
...
class BaseProducer:
    ...
    @retry(times=4, logger=logger, delay=2, backoff=5, on_fail=producer_on_fail)
    def produce_to_partition(self, topic: str, message: TMessage,
                             partition: int, on_delivery: Callable = None):
    ...
    @retry(times=4, logger=logger, delay=2, backoff=5, on_fail=producer_on_fail)
    def produce(self, topic: str, message: TMessage, on_delivery: Callable = None):
        content = json.dumps(message.to_dict())
        self.producer.produce(
            topic=topic,
            value=content,
    ...
    @retry(times=4, logger=logger, delay=2, backoff=5, on_fail=producer_on_fail)
    def sync_produce_to_partition(self, topic: str, message: TMessage,
                                  partition: int, on_delivery: Callable = None):
